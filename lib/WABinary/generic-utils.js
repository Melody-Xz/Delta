...
const assertNodeErrorFree = (node) => {
    ...
    if (errNode) {
        ...
        // SOLUCIÓN: Si el error es 403 (Forbidden), ignoramos el throw para que no crashee
        // Esto suele pasar al intentar editar mensajes enviados originalmente a un LID
        if (code === 403) {
            return;
        }
        throw new boom_1.Boom(errNode.attrs.text || 'Unknown error', { data: code });
    }
};
