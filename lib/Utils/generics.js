...
/** * CAMBIO APLICADO: 
 * Se modifica getKeyAuthor para filtrar LIDs y priorizar JIDs reales (@s.whatsapp.net / @c.us)
 */
const getKeyAuthor = (key, meId = 'me') => {
    if (key?.fromMe) return meId;
    
    // Priorizamos el participante (usualmente JID real en grupos)
    let author = key?.participant || key?.remoteJid || '';
    
    // Si el autor detectado es un LID, intentamos buscar el JID real en el otro campo
    if (typeof author === 'string' && author.endsWith('@lid')) {
        const altAuthor = key?.participant || key?.remoteJid || '';
        if (altAuthor && !altAuthor.endsWith('@lid')) {
            author = altAuthor;
        }
    }
