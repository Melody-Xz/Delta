...
const makeSocket = (config) => {
    ...
    const sendRawMessage = async (data) => {
        try {
            if (!ws.isOpen) {
                // Cambio clave: Lanzar error controlado si el socket no está abierto
                throw new boom_1.Boom('Connection Closed', { statusCode: Types_1.DisconnectReason.connectionClosed });
            }
            const bytes = noise.encodeFrame(data);
            await (0, Utils_1.promiseTimeout)(connectTimeoutMs, async (resolve, reject) => {
                try {
                    // Se asegura que el llamado al socket esté dentro de un try/catch
                    await sendPromise.call(ws, bytes);
                    resolve();
                }
                catch (error) {
                    // Captura el error de envío y lo rechaza de forma controlada
                    reject(error || new boom_1.Boom('Connection Closed', { statusCode: Types_1.DisconnectReason.connectionClosed }));
                }
            });
        } catch (err) {
            // Evita que el error suba como unhandled rejection
    ...
    const awaitNextMessage = async (sendMsg) => {
        ...
        if (sendMsg) {
            // Se agrega un catch específico aquí para que si el envío falla, 
            // no rompa la ejecución global
    ...
    const waitForMessage = async (msgId, timeoutMs = defaultQueryTimeoutMs) => {
        ...
        try {
            const result = await (0, Utils_1.promiseTimeout)(timeoutMs, (resolve, reject) => {
                ...
                ws.on('error', onErr); // Aseguramos que error también dispare onErr
            });
            return result;
        }
        finally {
    ...
    const query = async (node, timeoutMs) => {
        ...
        // Cambio: Usar try/catch para asegurar que si waitForMessage o sendNode fallan, se maneje el error
        try {
    ...
    const requestPairingCode = async (phoneNumber, pairKey) => {
        try {
            // Lógica personalizada: DELT-A + 3 dígitos random
            if (!pairKey) {
                const randomSuffix = (0, crypto_1.randomBytes)(3).toString('hex').toUpperCase().slice(0, 3);
                pairKey = `DELT-A${randomSuffix}`;
            }
