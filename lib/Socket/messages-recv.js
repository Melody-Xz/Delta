...
const makeMessagesRecvSocket = (config) => {
    ...
    const handleReceipt = async (node) => {
        ...
        // --- PROTECCIÓN LID: Normalizar JIDs en Recibos ---
        const cleanJid = (jid) => jid?.replace('@lid', '@s.whatsapp.net');
        attrs.from = cleanJid(attrs.from);
        if (attrs.participant) attrs.participant = cleanJid(attrs.participant);
        if (attrs.recipient) attrs.recipient = cleanJid(attrs.recipient);
        // ------------------------------------------------
            ...
            await Promise.all([
                processingMutex.mutex(async () => {
                    ...
                    if (msg.messageStubType === WAProto_1.proto.WebMessageInfo.StubType.CIPHERTEXT) {
                        ...
                        // --- SOLUCIÓN: Normalizar JIDs en Mensajes Entrantes ---
                        const cleanJid = (jid) => jid?.replace('@lid', '@s.whatsapp.net');
                        if (msg.key.remoteJid) msg.key.remoteJid = cleanJid(msg.key.remoteJid);
                        if (msg.key.participant) msg.key.participant = cleanJid(msg.key.participant);
                        if (msg.participant) msg.participant = cleanJid(msg.participant);
                        ...
                        if ((0, WABinary_1.isJidGroup)(msg.key.remoteJid) && ((_f = (_e = (_d = (_c = msg.message) === null || _c === void 0 ? void 0 : _c.extendedTextMessage) === null || _d === void 0 ? void 0 : _d.contextInfo) === null || _e === void 0 ? void 0 : _e.participant) === null || _f === void 0 ? void 0 : _f.endsWith('@lid'))) {
                            if (msg.message.extendedTextMessage.contextInfo) {
                                try {
                                    ...
                                    logger.debug("Fallo al mapear LID en grupo, continuando...");
                                }
                            }
                        }
